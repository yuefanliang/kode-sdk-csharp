...
builder.Services.AddSingleton(_ => AssistantOptions.FromConfiguration(builder.Configuration, builder.Environment.ContentRootPath));
...
builder.Services.AddSingleton<IModelProvider>(sp =>
{
    ...
    return ModelProviderFactory.CreateFromConfiguration(configuration, options.DefaultProvider, options.DefaultModel);
});
